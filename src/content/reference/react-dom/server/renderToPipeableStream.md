...
* `renderToPipeableStream`
  * renders a 
    * React tree | pipeable [Node.js Stream.](https://nodejs.org/api/stream.html) /
      * generate static HTML
        * if you want server-generated HTML interactive -> | client, call [`hydrateRoot`](../client/hydrateRoot)
  * Node.js-specific API
    * -> if the environment contains [Web Streams](https://developer.mozilla.org/en-US/docs/Web/API/Streams_API) -> use [`renderToReadableStream`](renderToReadableStream)
...
* `options`
  * == object /
    * OPTIONAL
    * contain streaming options
      * `bootstrapScriptContent: string`
        * OPTIONAL
        * placed | inline `<script>`
      * `bootstrapScripts: []string`
        * OPTIONAL
        * == URLs -- for -- `<script>` / emit | page
        * use case
          * include the `<script>` / calls [`hydrateRoot`](../client/hydrateRoot)
        * ❌if you do NOT want to run React | client -> omit it❌
      * `bootstrapModules`
        * OPTIONAL
        * == `bootstrapScripts`
          * BUT emits [`<script type="module">`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Guide/Modules)
      * `identifierPrefix`
        * OPTIONAL
        * TODO: string prefix React uses -- for -- IDs generated by [`useId`](/reference/react/useId)
        * use case: avoid conflicts | MULTIPLE roots | SAME page
        * ⚠️MUST be the same prefix as passed to [`hydrateRoot`](../client/hydrateRoot#parameters)⚠️
      * `namespaceURI`
        * OPTIONAL
        * string / root [namespace URI](https://developer.mozilla.org/en-US/docs/Web/API/Document/createElementNS#important_namespace_uris) -- for the -- stream
        * defaults to regular HTML
        * `'http://www.w3.org/2000/svg'` for SVG
        * `'http://www.w3.org/1998/Math/MathML'` for MathML
      * `nonce`
        * OPTIONAL
        * [`nonce`](http://developer.mozilla.org/en-US/docs/Web/HTML/Element/script#nonce) string / allow scripts -- for -- [`script-src` Content-Security-Policy](https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers/Content-Security-Policy/script-src)
      * `onAllReady`
        * OPTIONAL
        * callback / fires when ALL rendering is complete (both [shell](#specifying-what-goes-into-the-shell) + all additional [content](#streaming-more-content-as-it-loads))
        * use case: [crawlers and static generation](#waiting-for-all-content-to-load-for-crawlers-and-static-generation)
        * ⚠️if you start streaming here -> ❌NO progressive loading❌ — stream will contain the final HTML⚠️
      * `onError`
        * OPTIONAL
        * callback / fires whenever there is a server error ([recoverable](#recovering-from-errors-outside-the-shell) or [not](#recovering-from-errors-inside-the-shell))
        * by default: only calls `console.error`
        * use case: [log crash reports](#logging-crashes-on-the-server), [adjust the status code](#setting-the-status-code) before the shell is emitted
      * `onShellReady`
        * OPTIONAL
        * callback / fires right after the [initial shell](#specifying-what-goes-into-the-shell) has been rendered
        * here you can [set the status code](#setting-the-status-code) and call `pipe` to start streaming
        * React will [stream additional content](#streaming-more-content-as-it-loads) after the shell + inline `<script>` tags / replace HTML loading fallbacks
      * `onShellError`
        * OPTIONAL
        * callback / fires if there was an error rendering the initial shell
        * receives the error as argument
        * ❌NO bytes emitted from the stream yet❌
        * ❌neither `onShellReady` nor `onAllReady` will get called❌
        * use case: [output a fallback HTML shell](#recovering-from-errors-inside-the-shell)
      * `progressiveChunkSize`
        * OPTIONAL
        * number of bytes in a chunk. [Read more about the default heuristic.](https://github.com/facebook/react/blob/14c2be8dac2d5482fda8a0906a31d239df8551fc/packages/react-server/src/ReactFizzServer.js#L210-L225)

#### Returns {/*returns*/}

* 's return
  * object / contain 2 methods
    * `pipe`
      * outputs the HTML | provided [writable Node.js Stream.](https://nodejs.org/api/stream.html#writable-streams) 
      * use cases
        * if you want to enable streaming -> call it | `onShellReady`
        * if you want to enable for crawlers & static generation -> call it | `onAllReady` 
    * `abort` 
      * lets you 
        * [abort server rendering](#aborting-server-rendering-aborting-server-rendering)
        * render the rest | client
...
On the client, your bootstrap script should [hydrate the entire `document` with a call to `hydrateRoot`:](../client/hydrateRoot#hydrating-an-entire-document)
...
If retrying rendering `Posts` on the client succeeds, the loading fallback from the server will be replaced with the client rendering output. The user will not know that there was a server error. However, the server `onError` callback and the client [`onRecoverableError`](../client/hydrateRoot#hydrateroot) callbacks will fire so that you can get notified about the error.
