...
* `hydrateRoot`
  * == React's built-in React APIs /
    * lets you 
      * display React components | browser DOM node /
        * ⚠️HTML content was PREVIOUSLY generated -- by -- [`react-dom/server`](../server)⚠️
          * _Examples:_ NextJS, Remix, ...
  * use cases
    * | FULLY React app,
      * `hydrateRoot` is used 1!
...
* `domNode`
  * TODO: [DOM element](https://developer.mozilla.org/en-US/docs/Web/API/Element) that was rendered as the root element on the server.

* `reactNode`: The "React node" used to render the existing HTML
* This will usually be a piece of JSX like `<App />` which was rendered with a `ReactDOM Server` method such as `renderToPipeableStream(<App />)`.
