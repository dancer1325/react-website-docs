...
* `useContext`
  * == React Hook /
    * lets you
      * | your component,
        * read a [context](../../learn/passing-data-deeply-with-context) 
...
* `SomeContext`
  * context /
    * you've PREVIOUSLY created -- with -- [`createContext`](createContext)
...
* context value -- for the -- calling component /
  * ALWAYS up-to-date
  * passed -- to the -- closest `SomeContext.Provider` | ABOVE calling component | tree
    * if there is NO provider -> returned value == `defaultValue` / you have passed to [`createContext`](createContext) | that context 
    * if the providers changes it's values -> React AUTOMATICALLY re-renders the component
...
* Skipping re-renders with [`memo`](/reference/react/memo) does not prevent the children receiving fresh context values.
